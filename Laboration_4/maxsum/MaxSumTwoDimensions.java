 ...
 *
 * @authors Isak Gabriel Lindeby Isak Einler Larsson
 * @version 2017-04-30
 */
  ...
  public static int maxSubMatrixSumBad( int[][] a ) {
   ...
   return maxSum;
    ...
    public static int maxSubMatrixSumEvenBetter( int[][] a ) {
      int maxSum = 0;
      int thisSum = 0;
      int[] subSeqOneDim = new int[a.length];

      for(int c1 = 0; c1 < a.length; c1++){
        for(int c2 = c1; c2 < a.length; c2++){

          //adderar ny rad till endim array
          for(int r1 = 0; r1 < a.length; r1++){
            subSeqOneDim[r1] += a[r1][c2];
          }
          thisSum = 0;
          //hittar största summan i den endimissionella arrayen
          //med linjära metoden från MaxSumOneDimension.java
          for( int i = 0, j = 0; j < subSeqOneDim.length; j++ )
          {
              thisSum += subSeqOneDim[ j ];
        ...
        //nollställ endim array
        for(int i = 0; i < subSeqOneDim.length; i++){
          subSeqOneDim[i] = 0;
        }
      }
      return maxSum;
